...
const LivesReplays = () => {
  ...
  if (chargement) {
    return <div className="espace-loading">Chargement des événements...</div>;
  }
    ...
    <div className="lives">
      ...
      {aVenir.length > 0 && (
        <section className="lives-section">
          <h3 className="lives-section-titre">
            <HiStatusOnline style={{ color: 'var(--success)' }} /> À venir
          </h3>
          <div className="lives-grille">
            {aVenir.map((evt, i) => (
              <motion.article
                key={evt._id}
                className="lives-carte lives-carte-avenir"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.05 }}
              >
                <div className="lives-carte-header">
                  <span className="lives-type" style={{ color: couleurStatut[evt.statut] }}>
                    {iconType[evt.type]} {labelType[evt.type]}
                  </span>
                  <span className="lives-duree">
                    <HiClock /> {evt.duree} min
                  </span>
                </div>
                <h4 className="lives-titre">{evt.titre}</h4>
                <p className="lives-desc">{evt.description}</p>
                <div className="lives-date">
                  <HiCalendar /> {formatDate(evt.date)}
                </div>
                {evt.projet && (
                  <span className="lives-projet">Projet : {evt.projet.nom}</span>
                )}
              </motion.article>
            ))}
          </div>
      ...
      {termines.length > 0 && (
        <section className="lives-section">
          <h3 className="lives-section-titre">
            <HiPlay style={{ color: 'var(--muted)' }} /> Replays disponibles
          </h3>
          <div className="lives-grille">
            {termines.map((evt, i) => (
              <motion.article
                key={evt._id}
                className="lives-carte"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.05 }}
              >
                ...
                {evt.lienVideo && (
                  <a
                    ...
                    className="btn btn-primary lives-play-btn"
                  >
