...
import { requireStaff, requirePermission } from '../middlewares/checkUserStatus.js';
import {
  listerReports,
  traiterReport,
  getReportStats,
...
import { getDashboard, getMe } from '../controllers/dashboardController.js';
import {
  listUsers,
 ...
 * Toutes les routes necessitent authentification + permissions appropriees
 *
 * Mapping des routes pour l'outil de moderation:
 * - /api/admin/me -> info moderateur connecte
 * - /api/admin/dashboard -> stats dashboard
 * - /api/admin/dashboard/stats -> alias dashboard
 * - /api/admin/reports -> signalements
 * - /api/admin/users -> utilisateurs (liste, details, audit, timeline)
...
// GET /api/admin/me - Info du moderateur connecte
router.get('/me', verifierJwt, requireStaff, getMe);

// GET /api/admin/dashboard - Donnees du dashboard (agregees)
router.get('/dashboard', verifierJwt, requireStaff, getDashboard);

// GET /api/admin/dashboard/stats - Alias pour dashboard (compatibilite outil moderation)
...
// GET /api/admin/users/search - Recherche utilisateurs (meme endpoint avec query param)
...
// GET /api/admin/users/:id/timeline - Timeline de moderation d'un utilisateur
router.get('/users/:id/timeline', verifierJwt, requirePermission('users:view'), getUserModerationTimeline);

// GET /api/admin/users/:id/activity - Activite complete d'un utilisateur
router.get('/users/:id/activity', verifierJwt, requirePermission('users:view'), getUserActivity);

// GET /api/admin/users/:id/reports - Reports crees par l'utilisateur
...
// POST /api/admin/users/:id/unban - Debannir un utilisateur (alias)
router.post('/users/:id/unban', verifierJwt, requirePermission('users:unban'), unbanUser);

// PATCH /api/admin/users/:id/role - Changer le role d'un utilisateur (alias)
...
// GET /api/admin/reports/aggregated - Signalements agreges par cible
router.get('/reports/aggregated', verifierJwt, requirePermission('reports:view'), getAggregatedReports);

// GET /api/admin/reports/:id - Detail d'un signalement
...
// GET /api/admin/audit/:id - Detail d'un audit log
router.get('/audit/:id', verifierJwt, requirePermission('audit:view'), getAuditLog);

// ============ STAFF CHAT ============

// Original endpoints
// GET /api/admin/staff-chat - Recuperer les messages
...
// Alias endpoints pour l'outil de moderation (/api/admin/chat)
// GET /api/admin/chat - Recuperer les messages (alias)
