/**
 * Profil Controller — Main Profile Functions
 * modifierProfil, changerMotDePasse, supprimerCompte, modifierStatut, exporterDonnees
 */
...
export const modifierProfil = async (
  ...
  try {
    ...
    // Mettre a jour l'utilisateur (email excluded by guard above)
...
export const changerMotDePasse = async (
  ...
  try {
    ...
    // Verifier si l'utilisateur a un compte local
...
export const supprimerCompte = async (
  ...
  try {
    ...
    // Pour les comptes locaux, verifier le mot de passe
    if (utilisateur.provider === 'local' && utilisateur.motDePasse) {
    ...
    // SEC-RGPD-03: Supprimer TOUTES les donnees liees a l'utilisateur (cascade complete)
    await Promise.all([
      // Notifications (recues)
      Notification.deleteMany({ destinataire: userId }),
      // Notifications creees par l'utilisateur (data.userId)
    ...
    // Pour les conversations 1-1 ou l'utilisateur participe : supprimer
    ...
    for (const conv of conversationsUtilisateur) {
      if (!conv.estGroupe) {
        // Conversation privee : supprimer la conversation et ses messages
        ...
        if (conv.participants.length === 0) {
          ...
          // Transferer le createur si necessaire
...
export const modifierStatut = async (
  ...
  try {
    ...
    // Entrepreneur -> Visiteur : verifier et supprimer les projets
    if (utilisateur.statut === 'entrepreneur' && donnees.statut === 'visiteur') {
      ...
      if (projetsPublies.length > 0) {
        // Raison obligatoire si projets publies
        ...
        // Notifier les followers puis supprimer uniquement les projets publies (brouillons conserves)
        for (const projet of projetsPublies) {
          // Creer les notifications pour chaque follower
          ...
          // Supprimer le projet publie + cascade (sans toucher aux notifs de cloture qu'on vient de creer)
          await Promise.all([
