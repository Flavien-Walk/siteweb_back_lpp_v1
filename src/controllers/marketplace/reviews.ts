 ...
 */
export const creerReview = async (req: Request, res: Response) => {
  try {
    ...
    const commande = await MarketplaceOrder.findById(commandeId);
    if (!commande) {
    ...
    if (!commande.acheteur.equals((req as any).utilisateur._id)) {
    ...
    const review = await MarketplaceReview.create({
      ...
      commande: commandeId,
      auteur: (req as any).utilisateur._id,
      note,
      commentaire,
    });
 ...
 */
export const getReviewsService = async (req: Request, res: Response) => {
  try {
    ...
    const total = await MarketplaceReview.countDocuments({ service: serviceId });

    // Recuperer les avis avec pagination
    const reviews = await MarketplaceReview.find({ service: serviceId })
      .sort({ dateCreation: -1 })
      .skip(skip)
      .limit(limit)
 ...
 */
export const modifierReview = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { note, commentaire } = req.body;

    // Recuperer l'avis
    const review = await MarketplaceReview.findById(id);
    if (!review) {
      return res.status(404).json({
        succes: false,
    ...
    if (!review.auteur.equals((req as any).utilisateur._id)) {
      return res.status(403).json({
        succes: false,
    ...
    const dateCreation = new Date((review as any).dateCreation).getTime();
 ...
 */
export const supprimerReview = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    await MarketplaceOrder.findByIdAndUpdate(commandeId, { aReview: false });

    // Supprimer l'avis
    await MarketplaceReview.findByIdAndDelete(id);
