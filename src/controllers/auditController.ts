...
export const listAuditLogs = async (
  ...
  try {
    ...
    const targetType = req.query.targetType as AuditTargetType;
    if (targetType) {
...
/**
 * Obtenir un audit log spécifique
...
export const getAuditLog = async (
  ...
  try {
    ...
    if (!log) {
      throw new ErreurAPI('Log non trouvé', 404);
    }
...
/**
 * Obtenir l'historique d'audit d'une cible spécifique
...
export const getTargetHistory = async (
  ...
  try {
    ...
    const validTargetTypes = ['utilisateur', 'publication', 'commentaire', 'message', 'story', 'live', 'report', 'config', 'system'];
...
export const getAuditStats = async (
  ...
  try {
    ...
    const [actionStats, actorStats, dailyStats, totalLogs] = await Promise.all([
      ...
      // Top acteurs (modérateurs les plus actifs)
      AuditLog.aggregate([
        { $match: { dateCreation: { $gte: dateFrom } } },
...
export const exportAuditLogs = async (
  ...
  try {
    // Filtres identiques à listAuditLogs
    const filter: Record<string, unknown> = {};
