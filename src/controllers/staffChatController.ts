...
import StaffChat from '../models/StaffChat.js';
...
/**
 * Recuperer les messages du staff chat
...
export const getStaffMessages = async (
  ...
  try {
    ...
    if (before && mongoose.Types.ObjectId.isValid(before)) {
      const beforeMessage = await StaffChat.findById(before);
      if (beforeMessage) {
        filter.dateCreation = { $lt: beforeMessage.dateCreation };
      }
    }

    const messages = await StaffChat.find(filter)
      .sort({ dateCreation: -1 })
      .limit(limit)
...
export const sendStaffMessage = async (
  ...
  try {
    ...
    // Verifier le lien avec un report si fourni
    ...
    const message = await StaffChat.create({
      sender: senderId,
      type: linkedReport ? 'report_link' : 'text',
      content: donnees.content,
      linkedReport,
      readBy: [senderId], // L'expediteur a deja "lu" son message
    });

    // Populer les references pour la reponse
...
export const markMessagesAsRead = async (
  ...
  try {
    ...
    // Valider les IDs
    const validIds = messageIds.filter((id: string) =>
    ...
    // Ajouter l'utilisateur a readBy pour tous les messages
    await StaffChat.updateMany(
      {
        _id: { $in: validIds.map((id: string) => new mongoose.Types.ObjectId(id)) },
        readBy: { $ne: userId }, // Eviter les doublons
      },
      {
        $push: { readBy: userId },
      }
...
export const getUnreadCount = async (
  ...
  try {
    ...
    const count = await StaffChat.countDocuments({
      readBy: { $ne: userId },
    });
...
/**
 * Creer un message systeme (utilise par d'autres controleurs)
 */
export const createSystemMessage = async (
  ...
  try {
    ...
    console.error('[StaffChat] Erreur creation message systeme:', error);
  }
};
...
export const deleteStaffMessage = async (
  ...
  try {
    ...
    const message = await StaffChat.findById(messageId);
